...
namespace RPGCharacterAnims
{
    ...
    public class RPGCharacterMovementController:SuperStateMachine
    {
		...
		*/
        ...
        public float doubleJumpImpactSpeed;
        public Transform target;
        public bool isAimingToVine;
        public float inAirRunSpeed;

        public bool disableInSwing;
        ...
        private void Move_SuperUpdate()
        {
            ...
            if(rpgCharacterInputController.HasMoveInput() && canMove)
            {
                ...
                //Run.
                if (isAimingToVine && rpgCharacterInputController.moveInput != null)
                {
                    currentVelocity = Vector3.MoveTowards(currentVelocity, (target.position - this.transform.position) * runSpeed + rpgCharacterInputController.moveInput * inAirRunSpeed, movementAcceleration * superCharacterController.deltaTime);
                }
                else
                {
        ...
        public void ReleaseMovement()
        {
            ...
            //rpgCharacterController.enabled = false;
            jumpReset = false;
        }
        

        public void AcquireMovement()
        {
            disableInSwing = false;
            rb.isKinematic = true;
        ...
        {
            if (!rpgCharacterController.stopFall)
            {
                ...
                planarMoveDirection = Vector3.MoveTowards(planarMoveDirection, (target.position - this.transform.position)* inAirSpeed, jumpAcceleration * superCharacterController.deltaTime);
                ...
                if (rpgCharacterInputController.inputJump || jumpReset)
                {
                    currentVelocity = (target.position - this.transform.position) * doubleJumpImpactSpeed;
